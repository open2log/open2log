---
name: adr-suggester
description: Automatically suggests creating ADRs after implementing significant code changes. Triggers when adding dependencies, changing database schemas, modifying API endpoints, altering authentication, updating CI/CD pipelines, or introducing new architectural patterns. Use after completing implementation work.
allowed-tools: Read, Grep, Glob, Bash(adr:*), Bash(git diff:*), Bash(git log:*)
---
...
2. **Identify ADR-worthy decisions**
   Look for:
   - New files in `package.json`, `Cargo.toml`, `go.mod`, `requirements.txt`, etc.
...
The /adr command will create the ADR file and auto-commit it.
